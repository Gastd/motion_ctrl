...
def dict_to_string(obj, start_ident='', parents=RefPath()):
    start, end = '{ ', ' }'
    ident, line_break = start_ident+'  ', '\n'

    field_pars = ['{}{}:{}'.format(ident,k,obj_to_string(v, ident, parents=parents)) for k,v in obj.items()]
    return start + line_break.join(field_pars) + end

def set_to_string(obj, start_ident='', already_referenced=set()):
    ident, line_break = start_ident + '  ', '\n'
    return str(list(map( 
        lambda value: '{}'.format(obj_to_string(value, ident, already_referenced)) + line_break
        , obj)))
...
def obj_to_string(obj, base_ident='', ident='  ', parents=RefPath()):
    ...
    if not hasattr(obj, '__dict__'):
        raise Exception('non primitive not listed:{}'.format(obj.__class__))
    # object
    if obj in parents:
...
def key_value_to_string_base(get_keys, get_values, mapping_func=obj_to_string, ident='  ', start='{\n', end='}', line_break=',\n', base_ident=''):
    internal_ident = base_ident + ident
    def func(target, parents):
        field_pars = ['{}{}: {}'.format(internal_ident,k,mapping_func(get_values(target, k), base_ident=internal_ident, parents=parents )) for k in get_keys(target)]
...
def enumeration_to_string_base(mapping_func=obj_to_string, ident='  ', start='[', end=']', line_break=', ', base_ident=''):
    ...
    def func(target, parents):
        ...
        #     return '<>'
        # else:
        field_pars = ['{}'.format(mapping_func(k, base_ident=internal_ident, parents=parents)) for k in target]
        return start + line_break.join(field_pars) + end
    return func
